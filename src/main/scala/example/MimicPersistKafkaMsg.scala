...
object MimicPersistKafkaMsg extends App {
  ...
  case class CommitableOffset(offset: Long) extends Commitable {
    def commitOffset(implicit ec: ExecutionContext): Future[Done] = Future {
  ...
  def someBusinessFuture(a: Long)(implicit ec: ExecutionContext) = Future {
  ...
  val killSwitch = KillSwitches.shared("injectable switch")
  val injectedCancellable: BooleanCancelable = BooleanCancelable()
